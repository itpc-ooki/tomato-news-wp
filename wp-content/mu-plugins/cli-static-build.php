...
private static function sync_uploads_assets(): void {
  ...
  private static function build_varieties_json(string $paper): void {
    ...
    // Always write only the public build output.
    // Do not write generated data back into /static-src, because that folder is source/templates only.
...
'prefectures' => $prefectures,
    ...
    // livestream.json (WEB seminar live stream settings)
    // Generated by livestream-manager.php. Safe no-op if plugin not present.
    if (class_exists('Tomato_Livestream_Manager') && method_exists('Tomato_Livestream_Manager', 'export_json_for_paper')) {
      Tomato_Livestream_Manager::export_json_for_paper($paper);
    }
  }
