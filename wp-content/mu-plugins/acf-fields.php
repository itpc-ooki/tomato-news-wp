 ...
 *
 * Adds a safe "Duplicate" action for standard articles/posts in wp-admin.
 * The duplicated post is always created as a draft so published articles are not
 * accidentally published twice. No front-end HTML/CSS/JS is changed here.
 */
if (!function_exists('tomato_duplicate_admin_post_as_draft')) {
  function tomato_duplicate_admin_post_as_draft(): void {
    if (!is_admin()) {
    ...
    check_admin_referer('tomato_duplicate_post_' . $source_id);
    ...
    if (!current_user_can('edit_post', $source_id) || !current_user_can('edit_posts')) {
      wp_die(__('You do not have permission to duplicate this post.', 'tomato'));
    }
    ...
    if (is_wp_error($new_post_id) || !$new_post_id) {
      wp_die(__('Failed to duplicate the post.', 'tomato'));
    }
...
add_action('admin_action_tomato_duplicate_post', 'tomato_duplicate_admin_post_as_draft');
...
add_filter('post_row_actions', function ($actions, $post) {
  ...
  $url = wp_nonce_url(
    add_query_arg([
      'action' => 'tomato_duplicate_post',
      'post' => $post->ID,
    ], admin_url('admin.php')),
    'tomato_duplicate_post_' . $post->ID
  );

  $actions['tomato_duplicate_post'] = '<a href="' . esc_url($url) . '" aria-label="' . esc_attr__('Duplicate this post as a draft', 'tomato') . '">' . esc_html__('複製', 'tomato') . '</a>';
  return $actions;
}, 10, 2);

add_action('admin_notices', function () {
  if (!isset($_GET['tomato_duplicated']) || (int) $_GET['tomato_duplicated'] !== 1) {
    return;
  }
  echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Post duplicated as a draft.', 'tomato') . '</p></div>';
});
